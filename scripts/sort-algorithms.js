...
class sortAlgorithms {
    ...
    ShellSort = async () => {
        ...
        for (let gap = Math.floor(n / 2); gap > 0; gap = Math.floor(gap / 2)) {
            for (let i = gap; i < n; i++) {
                let temp = this.list[i].getAttribute("value");
                let j;
                for (j = i; j >= gap && Number(this.list[j - gap].getAttribute("value")) > Number(temp); j -= gap) {
                    await this.help.mark(j);
                ...
                this.list[j].setAttribute("value", temp);
                this.list[j].style.height = `${3.5 * temp}px`;
            }
        }
        for (let counter = 0; counter < this.size; ++counter) {
            this.list[counter].setAttribute("class", "cell done");
